...
interface Props {
  ...
  handleSubmit: (e: React.FormEvent, isMarked?: boolean) => string | undefined;
  // eslint-disable-next-line no-unused-vars
  ...
  return (
    ...
    >
      <div className="flex gap-2">
        <button
          className={`flex items-center justify-center h-10 w-10 mb-2 rounded-xl ${
            isMarked
              ? "bg-yellow-500 text-stone-800"
              : "bg-stone-800 text-yellow-600"
          }`}
          onClick={() => setIsMarked(!isMarked)}
        >
          <BookmarkFilledIcon className="text-inherit bg-inherit" />
        </button>
        <div className="w-full">
          {pageIsSubmit && (
            <form
              onSubmit={(e) => handleSubmit(e, isMarked)}
